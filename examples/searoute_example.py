# %% [markdown]
# ## Oil Shipping Network Visualization with Sea Routes <br>

# Simulates global oil tanker traffic between major oil ports/regions. <br>
# Demonstrates geom_searoute with realistic maritime shipping lanes. <br>
# %%
import numpy as np
import pandas as pd
from ggplotly import ggplot, aes, geom_searoute, geom_map, theme_dark, labs
...
# %%
# =============================================================================
# Get unique routes for visualization (reduces computation time)
# =============================================================================

# Since many routes repeat, get unique source-target pairs
unique_routes = edges_df[['x', 'y', 'xend', 'yend']].drop_duplicates()
print(f"\nUnique routes to compute: {len(unique_routes)} (from {len(edges_df)} total movements)")

# %%
# =============================================================================
# Visualize with sea routes
# =============================================================================

print("\n" + "="*60)
print("Creating visualization with realistic sea routes...")
print("Routes follow actual maritime shipping lanes")
print("="*60 + "\n")

# Create the visualization
fig = (
    ggplot(unique_routes, aes(x='x', y='y', xend='xend', yend='yend'))
    + geom_map(
        map_type='world',
        landcolor='#1a1a2e',
        oceancolor='#0f0f1a',
    ...
    + labs(
        ...
        subtitle=f'{len(unique_routes)} unique sea routes (from {n_edges} simulated vessel movements)'
    )
)
...
figure.update_layout(
    ...
    title=dict(
        text=f'<b>Global Oil Tanker Shipping Network</b><br><sup>{len(unique_routes)} sea routes | Realistic maritime lanes via searoute</sup>',
...
top_flows = regional_flows.head(10).reset_index()
top_flows.columns = ['source_region', 'target_region', 'flow']
for _, row in top_flows.iterrows():
    print(f"  {row['source_region']:15} -> {row['target_region']:15}: {row['flow']:.1f}M DWT")
