...
class TestGeomPacf:
    ...
    def test_basic_usage(self):
        ...
        plot = ggplot(df, aes(y='value')) + geom_pacf()
        fig = plot.draw()
    ...
    def test_custom_nlags(self):
        ...
        plot = ggplot(df, aes(y='value')) + geom_pacf(nlags=20)
        fig = plot.draw()
    ...
    def test_custom_color(self):
        ...
        plot = ggplot(df, aes(y='value')) + geom_pacf(color='coral')
        fig = plot.draw()
    ...
    def test_starts_at_lag_one(self):
        """Test that PACF starts at lag 1 (lag 0 excluded)."""
        np.random.seed(42)
        df = pd.DataFrame({'value': np.random.randn(100).cumsum()})

        plot = ggplot(df, aes(y='value')) + geom_pacf(nlags=10)
        fig = plot.draw()
    ...
    def test_custom_method(self):
        """Test different PACF computation methods."""
        np.random.seed(42)
        df = pd.DataFrame({'value': np.random.randn(100).cumsum()})

        # Test with OLS method
        plot = ggplot(df, aes(y='value')) + geom_pacf(method='ols', nlags=10)
        fig = plot.draw()
