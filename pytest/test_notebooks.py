...
def get_doc_notebook_id(notebook_path: Path) -> str:
    """Generate a readable test ID from notebook path."""
    return str(notebook_path.relative_to(DOCS_DIR))


@pytest.mark.parametrize("notebook_path", NOTEBOOKS, ids=get_doc_notebook_id)
def test_doc_notebook_executes(notebook_path: Path, tmp_path: Path):
...
EX_NOTEBOOKS = list(EXAMPLES_DIR.glob("**/*.ipynb"))


def get_ex_notebook_id(notebook_path: Path) -> str:
    """Generate a readable test ID from notebook path."""
    return str(notebook_path.relative_to(EXAMPLES_DIR))


@pytest.mark.parametrize("notebook_path", EX_NOTEBOOKS, ids=get_ex_notebook_id)
def test_ex_notebook_executes(notebook_path: Path, tmp_path: Path):
    """Test that a notebook executes without errors.
